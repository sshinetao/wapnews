...
function page($count, $page_size, $num_btn = 4, $page = 'page') {
	...
	if ($num_btn >= $page_num_all) {
		for($i = 1; $i <= $page_num_all; $i ++) {
			if ($_GET [$page] == $i) {
				$html [$i] = "<li class='am-active'><a href='#'>{$i}</strong></a></li>";//当前页
			} else {
		...
		for($i = 0; $i < $num_btn; $i ++) {
			if ($_GET [$page] == $start) {
				$html [$start] = "<li class='am-disabled'><a href='#'>{$start}</strong></a></li>";
			} else {
	...
	if ($_GET [$page] != 1) {
		$prev = $_GET [$page] - 1;
		array_unshift ( $html, "<li><a href='{$url}{$prev}' ><span class='am-icon-angle-double-left'></a></li>" );
	}
	if ($_GET [$page] != $page_num_all) {
		$next = $_GET [$page] + 1;
		array_push ( $html, " <li><a href='{$url}{$next}' '><span class='am-icon-angle-double-right'></a></li>" );
	}
	$html = implode ( '  ', $html );
	$data = array (
			'limit' => $limit,
